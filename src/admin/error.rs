...
impl fmt::Display for AdminServiceError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            AdminServiceError::NotLeader {
                instance_id,
                leader_id: Some(leader_id),
            } => write!(
                f,
                "当前实例不是运行时 leader，请将写请求发送到 leader（instanceId={}, leaderId={}）",
            ...
            AdminServiceError::NotLeader {
                ...
                f,
                "当前未观察到运行时 leader，暂时拒绝写请求（instanceId={}）",
                instance_id
            ),
...
impl AdminServiceError {
    ...
    pub fn into_response(self) -> AdminErrorResponse {
        match &self {
            ...
            AdminServiceError::NotLeader {
                leader_id: Some(_), ..
            } => AdminErrorResponse::new("conflict_error", self.to_string()),
            AdminServiceError::NotLeader {
                leader_id: None, ..
            } => AdminErrorResponse::new("service_unavailable", self.to_string()),
