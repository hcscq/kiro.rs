...
| `suspiciousActivityCooldownMs` | number | `7200000` | 单账号触发 Kiro `suspicious activity` 临时限制后的账号级全局冷却时间（毫秒）；`0` 表示不写入固定冷却 |
| `suspiciousActivityCooldownEnabled` | boolean | `true` | 是否启用 `suspicious activity` 独立长冷却；独立于普通 `rateLimitCooldownEnabled` |
| `suspiciousActivityPreferCleanCredentials` | boolean | `true` | 是否优先调度从未触发 `suspicious activity` 的账号；命中过的账号仅作为兜底候选 |
...
```json
{
   ...
   "suspiciousActivityCooldownMs": 7200000,
   "suspiciousActivityCooldownEnabled": true,
   "suspiciousActivityPreferCleanCredentials": true,
